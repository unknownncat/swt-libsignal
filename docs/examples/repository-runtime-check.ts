...
export interface RepositoryRuntimeCheckResult {
  readonly oneToOneOk: boolean
  readonly groupOk: boolean
  readonly addressString: string
}

export async function runRepositoryRuntimeCheck(): Promise<RepositoryRuntimeCheckResult> {
  await initCrypto()
  ...
  const direct = await aliceRepo.encryptMessage({
    jid: 'bob.1',
    data: new TextEncoder().encode('docs-repository-direct'),
  })
  const directOpened = await bobRepo.decryptMessage({
    jid: 'alice.1',
    type: 'pkmsg',
    ciphertext: direct.ciphertext,
  })

  const groupData = await aliceRepo.encryptGroupMessage({
    group: 'docs-group-repo',
    meId: 'alice.1',
    data: new TextEncoder().encode('docs-repository-group'),
  })
  await bobRepo.processSenderKeyDistributionMessage({
    groupId: 'docs-group-repo',
    authorJid: 'alice.1',
    axolotlSenderKeyDistributionMessage: groupData.senderKeyDistributionMessage,
  })
  const groupOpened = await bobRepo.decryptGroupMessage({
    group: 'docs-group-repo',
    authorJid: 'alice.1',
    msg: groupData.ciphertext,
  })

  return {
    oneToOneOk: new TextDecoder().decode(directOpened) === 'docs-repository-direct',
    groupOk: new TextDecoder().decode(groupOpened) === 'docs-repository-group',
    addressString: aliceRepo.jidToSignalProtocolAddress('bob.1'),
  }
}
